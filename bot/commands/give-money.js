...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('give-money')
    .setDescription('[Management] Geef geld aan een speler')
    .addUserOption((opt) =>
      opt.setName('discord').setDescription('De Discord gebruiker').setRequired(true)
    )
    .addIntegerOption((opt) =>
      opt.setName('bedrag').setDescription('Bedrag om toe te voegen').setRequired(true).setMinValue(1)
    )
    .addStringOption((opt) =>
      opt
        .setName('rekening')
    ...
    try {
      ...
      await interaction.editReply({
        embeds: [
          embeds.success(
            'Geld toegevoegd',
            `**${amount.toLocaleString('nl-NL')}** toegevoegd aan **${account}** van <@${target.id}> (${verification.robloxUsername}).\nCommand ID: \`${command.id}\` — wordt uitgevoerd zodra de speler online is.`
          ),
        ],
      });
      ...
      await logger.auditLog(client, {
        ...
        details: `+${amount} naar ${account} door <@${interaction.user.id}>`,
      });
    } catch (err) {
      await interaction.editReply({
