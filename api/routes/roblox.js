...
// commands like announce/shutdown). The API returns pending commands for those
// players and atomically marks them as "processing" so they are never delivered
// twice, even if multiple server instances poll around the same time.
router.post(
  '/poll',
  asyncHandler(async (req, res) => {
    ...
    const pendingResult = await db.execute({
      sql: `SELECT * FROM commands WHERE status = 'pending' AND roblox_id IN (${placeholders}) ORDER BY created_at ASC`,
      args: targets,
    });

    const claimed = [];

    // Claim each command with a conditional UPDATE — only one caller can
    // successfully flip status pending -> processing for a given row, so
    // this stays safe even if two server instances poll concurrently.
    for (const row of pendingResult.rows) {
      const claimResult = await db.execute({
        sql: `UPDATE commands SET status = 'processing' WHERE id = ? AND status = 'pending'`,
        args: [row.id],
      });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const commandResult = await db.execute({ sql: 'SELECT * FROM commands WHERE id = ?', args: [id] });
    ...
    await db.execute({
      sql: `UPDATE commands SET status = ?, executed_at = ?, result = ? WHERE id = ?`,
      args: [success ? 'completed' : 'failed', now, resultText, id],
    });
    ...
    if (command.type === 'ban' && success) {
      ...
      await db.execute({
        sql: `INSERT INTO bans (roblox_id, reason, banned_by, active, created_at) VALUES (?, ?, ?, 1, ?)`,
        args: [command.roblox_id, payload.reason || null, command.created_by, now],
      });
    }
    if (command.type === 'unban' && success) {
      await db.execute({
        sql: `UPDATE bans SET active = 0 WHERE roblox_id = ? AND active = 1`,
        args: [command.roblox_id],
      });
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await db.execute({
      sql: `INSERT INTO servers (job_id, players, max_players, last_heartbeat)
            VALUES (?, ?, ?, ?)
      ...
      args: [jobId, players, maxPlayers, now],
    });
